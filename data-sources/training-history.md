...
| ninja-turn | Ninja Turn | transition | primal_movement | beginner_intermediate | coordination, trunk_control, shoulder_support | Break the move into phases instead of rushing the turn. | hand_support, controlled_rotation | momentum_only, unstable_hands | segmented_turn -> continuous_transition | [Step by Step Tutorial to Master the Ninja Turn as a Beginner](https://www.youtube.com/watch?v=i4ZYqyREtus) | `animal flow ninja turn transition tutorial` |
...
| spinal-wave | Spinal Wave | flow_drill | animal_flow | beginner_intermediate | spinal_mobility, segmental_control | Move slowly at first to feel each segment of the spine. | wave_starts_at_hips, arms_stay_stable, continuous_motion, gentle_neck_never_force | rushing_the_wave, moving_from_arms_not_spine, forcing_the_neck | segmented_wave -> continuous_wave -> flowing_wave | [Beast Wave Tutorial](https://www.youtube.com/watch?v=JQUf-7NJ3KY) | `spinal wave primal movement` |
...
```json
{
  ...
  "execution_steps": [
    ...
    "Plant the reaching hand wide, outside the shoulder line, to protect the shoulder as it lands.",
...
```json
{
  ...
  "execution_steps": [
    ...
    "Continue the motion into the neck and head with a gentle nod — never force or crank the neck through the wave.",
    "Reverse the wave from the head back down through the spine.",
  ...
  "coach_notes": [
    "AI-drafted execution steps — review before relying on this as coaching content.",
    "keep neck movement gentle at all times — do not force range through the cervical spine"
  ]
}
```
